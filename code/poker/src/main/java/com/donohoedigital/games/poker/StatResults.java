...
public class StatResults extends LinkedHashMap
{
    public String toHTML()
    {
        ...
        Object key;
        String sName;
        buf.append(PropertyConfig.getMessage("msg.simresult.header"));
        while (iter.hasNext())
        {
            key = iter.next();

            StatResult result = (StatResult)get(key);

            if (key instanceof HandGroup) {
                sName = ((HandGroup)key).getName();
            ...
            buf.append(PropertyConfig.getMessage("msg.simresult.row",
                       Utils.encodeHTML(sName),
                       result.getHole().toStringRankSuit(),
